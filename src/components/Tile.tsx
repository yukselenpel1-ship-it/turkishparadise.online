...
export const Tile: React.FC<TileProps> = ({
  ...
  return (
    <div
      ...
      className={`relative flex flex-col justify-between rounded-md sm:rounded-xl border border-slate-700/80 hover:border-amber-400 transition-all duration-200 cursor-pointer select-none overflow-visible group shadow-md ${
        isChance
    ...
    >
      ...
      {owner && (
        <div
          className="absolute -top-1 -right-1 sm:-top-1.5 sm:-right-1.5 z-20 w-3.5 h-3.5 sm:w-5 sm:h-5 rounded-full border border-white/90 shadow-md flex items-center justify-center text-[8px] sm:text-[11px] animate-fade-in"
      ...
      {isProperty && colorBg && (
        <div className={`h-1.5 sm:h-3.5 w-full ${colorBg} shrink-0 flex items-center justify-center px-0.5 sm:px-1 shadow-sm rounded-t-sm sm:rounded-t-lg`}>
          {tile.houses > 0 && (
            <div className="flex items-center gap-0.5 drop-shadow">
              {tile.houses === 5 ? (
                <span className="text-[8px] sm:text-[10px] font-bold leading-none">🏨</span>
              ) : (
                <span className="text-[7px] sm:text-[9px] font-bold leading-none">{'🏠'.repeat(tile.houses)}</span>
              )}
            </div>
          )}
        </div>
      ...
      {/* 2. Station Header */}
      {isStation && (
        <div className="h-1.5 sm:h-3.5 w-full bg-cyan-600 shrink-0 flex items-center justify-center rounded-t-sm sm:rounded-t-lg">
          <Anchor className="w-2 h-2 sm:w-2.5 sm:h-2.5 text-white drop-shadow" />
        </div>
      )}

      {/* 3. City Landmark Photo Background */}
      {tile.image && (
        <div className="absolute inset-0 z-0 opacity-25 group-hover:opacity-50 transition-opacity duration-300 pointer-events-none rounded-md sm:rounded-xl overflow-hidden">
          <img
            src={tile.image}
            alt={tile.name}
      ...
      {isChance && (
        <div className="flex-1 flex flex-col items-center justify-center p-0.5 sm:p-1 text-center relative z-10">
          ...
          <span className="text-sm sm:text-3xl font-black text-white drop-shadow-md my-0.5 animate-bounce-short leading-none">
            ?
          </span>
        </div>
      )}
      ...
      {isChest && (
        <div className="flex-1 flex flex-col items-center justify-center p-0.5 sm:p-1 text-center relative z-10">
          ...
          <span className="text-xs sm:text-2xl drop-shadow-md my-0.5 animate-pulse leading-none">
            📦
          </span>
        </div>
      )}
      ...
      {isStart && (
        <div className="flex-1 flex flex-col items-center justify-center p-0.5 sm:p-1 text-center relative z-10">
          <span className="text-xs sm:text-2xl font-black text-slate-950 mb-0.5 leading-none">➔</span>
          <span className="text-[6.5px] sm:text-xs font-black text-slate-950 tracking-tight leading-none uppercase">
            BAŞLANGIÇ
          </span>
          <span className="text-[6px] sm:text-[9px] font-bold text-slate-900 mt-0.5 leading-none">
            +₺200
          </span>
        </div>
      )}
      ...
      {isJail && (
        <div className="flex-1 flex flex-col items-center justify-center p-0.5 sm:p-1 text-center relative z-10">
          <span className="text-xs sm:text-xl mb-0.5 leading-none">🔒</span>
          <span className="text-[6.5px] sm:text-[10px] font-bold text-white tracking-tight leading-none uppercase">
            HAPİS
          </span>
          <span className="text-[6px] sm:text-[8px] text-slate-300 leading-none">Ziyaret</span>
        </div>
      )}

      {isParking && (
        <div className="flex-1 flex flex-col items-center justify-center p-0.5 sm:p-1 text-center relative z-10">
          <span className="text-xs sm:text-xl mb-0.5 leading-none">🅿️</span>
          <span className="text-[6.5px] sm:text-[10px] font-bold text-white tracking-tight leading-none uppercase">
            OTOPARK
          </span>
        </div>
      ...
      {(isProperty || isStation) ? (
        <div className="flex-1 flex flex-col justify-between items-center p-0.5 sm:p-1 text-center relative z-10 min-h-0 w-full">
          ...
          <div className="w-full flex flex-col items-center shrink-0">
            <span className="font-bold text-[7px] sm:text-[10.5px] md:text-xs text-white tracking-tight leading-tight group-hover:text-amber-300 transition drop-shadow uppercase truncate w-full text-center px-0.5">
              {tile.name}
            </span>

            {tile.price && (
              <div className="inline-flex items-center gap-0.5 mt-0.5 bg-slate-950/85 px-1 py-0.2 rounded border border-slate-700">
                <span className="text-[6.5px] sm:text-[9.5px] font-bold text-amber-300 leading-none">
                  ₺{tile.price}
                </span>
              </div>
            )}
      ...
      {/* 8. Mortgaged Overlay */}
      {tile.isMortgaged && (
        <div className="absolute inset-0 bg-slate-950/90 z-20 flex items-center justify-center text-[7px] sm:text-[10px] font-bold text-amber-400 uppercase tracking-widest backdrop-blur-[2px] rounded-md sm:rounded-xl">
          İPOTEKLİ
        </div>
      )}
