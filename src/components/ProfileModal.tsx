...
export const ProfileModal: React.FC<ProfileModalProps> = ({
  ...
  // Subscribe to Real-Time Incoming Friend Requests & Presence
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-md animate-fade-in font-['Plus_Jakarta_Sans',sans-serif] select-none">
      <div className="bg-[#0a1020]/95 border border-amber-500/40 rounded-3xl max-w-lg w-full p-4 sm:p-6 text-left shadow-2xl relative overflow-hidden ring-1 ring-amber-500/20 space-y-3.5 max-h-[92vh] flex flex-col">
        ...
        <div className="flex items-center gap-3.5 pr-8 border-b border-slate-800/80 pb-3">
          <div className="relative shrink-0">
            {userAccount.photoURL ? (
              <img
                src={userAccount.photoURL}
                alt={userAccount.displayName}
                className="w-13 h-13 sm:w-15 sm:h-15 rounded-2xl object-cover border-2 border-amber-400/80 shadow-lg shadow-amber-500/20"
              />
            ) : (
              <div className="w-13 h-13 sm:w-15 sm:h-15 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-700 text-slate-950 font-black text-xl flex items-center justify-center border-2 border-amber-300 shadow-lg shadow-amber-500/20">
                {userAccount.displayName.charAt(0).toUpperCase()}
              </div>
            )}
            <div className="absolute -bottom-1 -right-1 bg-emerald-500 w-4 h-4 rounded-full border-2 border-slate-950 ring-1 ring-emerald-400" title="Çevrimiçi" />
          </div>
          ...
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <p className={`text-[11px] sm:text-xs font-bold bg-gradient-to-r ${rank.color} bg-clip-text text-transparent truncate`}>
              {rank.title}
            </p>
            
            {userAccount.friendCode && (
              <div className="flex items-center gap-1.5 pt-0.5">
                <span className="text-[10px] text-slate-400 font-medium">Özel ID:</span>
                <span className="text-[11px] font-mono font-black text-amber-300 bg-amber-500/10 border border-amber-500/30 px-1.5 py-0.2 rounded">
                  {userAccount.friendCode}
                </span>
              </div>
            )}
        ...
        {/* Tab Switcher: İstatistikler vs Arkadaşlarım */}
        <div className="flex items-center gap-1 p-1 bg-slate-900/90 rounded-2xl border border-slate-800">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-xl font-bold text-xs transition cursor-pointer ${
              activeTab === 'stats'
          ...
          >
            ...
            <span>İstatistikler & Başarımlar</span>
          </button>
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-xl font-bold text-xs transition cursor-pointer relative ${
              activeTab === 'friends'
                ? 'bg-amber-500 text-slate-950 shadow-md shadow-amber-500/20'
                : 'text-slate-400 hover:text-white'
            }`}
        ...
        {statusMessage && (
          <div className={`p-2.5 rounded-xl text-xs font-bold animate-fade-in flex items-center gap-1.5 border ${
        ...
        {activeTab === 'stats' && (
          <div className="overflow-y-auto pr-0.5 space-y-3.5 flex-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <div className="bg-[#070b14] border border-amber-500/30 rounded-2xl p-2.5 text-center space-y-0.5">
                <span className="text-[10px] text-amber-400 font-bold block uppercase tracking-wide">Zaferler</span>
                <span className="text-xl font-black text-amber-300">{stats.gamesWon}</span>
                <span className="text-[9px] text-slate-500 block font-semibold">1.lik</span>
              </div>

              <div className="bg-[#070b14] border border-slate-800 rounded-2xl p-2.5 text-center space-y-0.5">
              ...
              <div className="bg-[#070b14] border border-slate-800 rounded-2xl p-2.5 text-center space-y-0.5">
                ...
                <span className="text-xl font-black text-rose-400">{stats.gamesLost || 0}</span>
                <span className="text-[9px] text-slate-500 block font-semibold">İflas & Kayıp</span>
              </div>

              <div className="bg-[#070b14] border border-emerald-500/30 rounded-2xl p-2.5 text-center space-y-0.5">
                <span className="text-[10px] text-emerald-400 font-bold block uppercase tracking-wide">Toplam Kazanç</span>
                <span className="text-base sm:text-lg font-black text-emerald-300 truncate block">₺{stats.totalMoneyEarned.toLocaleString('tr-TR')}</span>
        ...
        {activeTab === 'friends' && (
          <div className="overflow-y-auto pr-0.5 space-y-3 flex-1">
            ...
            {isGoogleUser && userAccount.friendCode ? (
              <div className="bg-gradient-to-r from-amber-500/20 via-[#0e172e] to-sky-500/10 border border-amber-500/40 rounded-2xl p-3 space-y-1.5 relative overflow-hidden">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs transition cursor-pointer shadow-md active:scale-95 shrink-0"
                  >
              ...
              <div className="bg-gradient-to-r from-amber-500/10 to-rose-500/10 border border-amber-500/30 rounded-2xl p-3.5 text-center space-y-2">
                ...
                {onGoogleLogin && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white hover:bg-slate-100 text-slate-900 font-black text-xs transition cursor-pointer shadow active:scale-95"
                  >
            ...
            {/* 2. Gelen Arkadaşlık İstekleri (Varsa Gösterilir) */}
            {incomingRequests.length > 0 && (
              <div className="bg-amber-500/10 border border-amber-500/40 rounded-2xl p-3 space-y-2">
                <div className="flex items-center justify-between text-xs font-black text-amber-300 uppercase tracking-wide">
                  <span className="flex items-center gap-1.5">
                    <UserPlus className="w-3.5 h-3.5 text-amber-400" /> Gelen Arkadaşlık İstekleri
                  </span>
                ...
                <div className="space-y-1.5">
                  {incomingRequests.map((req) => (
                    <div
                      key={req.id}
                      className="p-2.5 rounded-xl bg-slate-950/90 border border-amber-500/30 flex items-center justify-between gap-2 text-xs"
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        {req.fromPhotoURL ? (
                          <img src={req.fromPhotoURL} alt="" className="w-7 h-7 rounded-xl object-cover shrink-0" />
                        ) : (
                        ...
                        <div className="min-w-0 space-y-0.2">
                          <p className="font-bold text-white truncate">{req.fromDisplayName}</p>
                          <p className="text-[9.5px] font-mono text-amber-300/90">{req.fromFriendCode}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-[11px] transition cursor-pointer shadow active:scale-95"
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-2 py-1 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 font-bold text-[11px] transition cursor-pointer active:scale-95"
                          title="İsteği Reddet"
                        >
                          <X className="w-3 h-3" />
                        </button>
            ...
            {/* 3. Özel ID ile Arkadaşlık İsteği Gönderme Formu */}
            {isGoogleUser && (
              <form onSubmit={handleSendRequest} className="space-y-1.5">
            ...
            {/* 4. Arkadaş Listesi & Çevrimiçi Durum & Odaya Katıl */}
              ...
              {friendsList.length === 0 ? (
                <div className="p-4 rounded-2xl bg-[#070b14] border border-slate-800/80 text-center space-y-1">
                  ...
                  <p className="text-[10px] text-slate-500">
                    Arkadaşınızın Özel ID'sini (Örn: TP-849201) yukarıya yazarak istek gönderebilirsiniz. İstek kabul edildiğinde burada listelenir.
                  </p>
                </div>
              ) : (
                  ...
                  {friendsList.map((friend) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0 space-y-0.5">
                            <div className="font-bold text-white flex items-center gap-1.5 truncate">
                              ...
                              {isOnline && (
                                <span className="text-[9px] bg-emerald-500/20 text-emerald-300 font-bold px-1.5 py-0.2 rounded border border-emerald-500/30 shrink-0 flex items-center gap-0.5">
                        ...
                        {/* Actions */}
                        <div className="flex items-center gap-1.5 shrink-0">
                          {/* Join Friend's Active Room Button (If Friend is Online & In Room) */}
                          {inActiveRoom && friend.activeRoomId && onJoinRoom ? (
                            <button
                              onClick={() => handleJoinFriendRoom(friend.activeRoomId)}
                              className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-slate-950 font-black text-[11px] transition cursor-pointer shadow-md shadow-emerald-500/20 active:scale-95 animate-pulse"
