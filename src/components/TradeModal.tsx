...
import { X, ArrowLeftRight, Coins, Building2, Check, AlertCircle } from 'lucide-react';
...
export const TradeModal: React.FC<TradeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-md animate-fade-in select-none">
      <div className="bg-[#0b1325] border border-slate-700/80 rounded-3xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="p-4 px-6 flex items-center justify-between border-b border-slate-800 bg-slate-900/90 shrink-0">
          <div className="flex items-center gap-2 text-amber-400 font-extrabold text-lg">
            ...
            <span>Mülk & Para Takas Masası</span>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-5">
          ...
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-wider text-slate-300 block">
              1. Takas Yapılacak Oyuncuyu Seçin
            </label>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-amber-300 font-extrabold">₺{p.money}</div>
                      </div>
                    </div>
            ...
            <div className="bg-slate-950/80 p-4 rounded-2xl border border-slate-800 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
                <span className="text-xs font-black uppercase tracking-wide text-emerald-400 flex items-center gap-1.5">
                  <Building2 className="w-4 h-4" /> Sizin Vereceğiniz Mülkler
                </span>
                <span className="text-[10px] text-slate-400 font-bold">
                  {selectedMyTileIds.length} Seçili
                </span>
              </div>
              ...
              <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1 min-h-[100px]">
                {myProperties.length === 0 ? (
                  <div className="text-center py-6 text-xs text-slate-500 font-semibold">
                    Henüz sahip olduğunuz mülk yok.
                  </div>
                ) : (
                  myProperties.map((tile) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-amber-300 font-bold">₺{tile.price}</span>
                      </div>
                    );
                  })
                )}
              ...
              <div className="pt-2 border-t border-slate-800/80">
                <label className="text-[11px] font-bold text-slate-300 flex items-center justify-between mb-1">
                  <span>Ekstra Teklif Edilecek Para</span>
                  <span className="text-emerald-400 font-bold">Bakiye: ₺{currentPlayer.money}</span>
                </label>
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-amber-400 font-black text-xs">
                    ₺
                  </span>
                  <input
                    type="number"
                    ...
                    placeholder="0"
                    className="w-full bg-slate-900 border border-slate-800 focus:border-amber-400 text-white rounded-xl pl-8 pr-3 py-2 text-xs font-bold outline-none"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-950/80 p-4 rounded-2xl border border-slate-800 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
                <span className="text-xs font-black uppercase tracking-wide text-amber-400 flex items-center gap-1.5">
                  <Building2 className="w-4 h-4" /> Karşıdan İstediğiniz Mülkler
                </span>
                <span className="text-[10px] text-slate-400 font-bold">
                  {selectedTargetTileIds.length} Seçili
                </span>
              </div>
              ...
              <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1 min-h-[100px]">
                {targetProperties.length === 0 ? (
                  <div className="text-center py-6 text-xs text-slate-500 font-semibold">
                    {targetPlayer?.name || 'Seçilen oyuncunun'} henüz mülkü yok.
                  </div>
                ) : (
              ...
              <div className="pt-2 border-t border-slate-800/80">
                <label className="text-[11px] font-bold text-slate-300 flex items-center justify-between mb-1">
                  <span>Karşıdan İstenecek Para</span>
                  <span className="text-amber-300 font-bold">Bakiyesi: ₺{targetPlayer?.money || 0}</span>
                </label>
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-amber-400 font-black text-xs">
                    ₺
          ...
          <div className="pt-2">
            ...
            >
              ...
              <span>Takas Teklifini Gönder</span>
            </button>
          </div>
